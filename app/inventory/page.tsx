...
export default function InventoryPage() {
  ...
  const fetchInventory = async () => {
    try {
      const response = await axios.get('http://localhost:8085/api/inventory');
  ...
  const fetchTransactions = async () => {
    try {
      const response = await axios.get('http://localhost:8085/api/inventory/transactions');
  ...
  const handleAddInventory = async (newInventory: { bloodGroup: string; quantity: number; minThreshold: number }) => {
    try {
      await axios.post('http://localhost:8085/api/inventory', newInventory);
  ...
  const handleDonation = async (bloodGroup: string, quantity: number) => {
    try {
      await axios.post(`http://localhost:8085/api/inventory/${bloodGroup}/donate`, { quantity });
      fetchInventory();
      fetchTransactions();
    } catch (error) {
  ...
  const handleRequest = async (bloodGroup: string, quantity: number) => {
    try {
      const checkAvailability = await axios.get(
        `http://localhost:8085/api/inventory/${bloodGroup}/check?quantity=${quantity}`
      );
      ...
      await axios.post(`http://localhost:8085/api/inventory/${bloodGroup}/request`, { quantity });
      fetchInventory();
      fetchTransactions();
    } catch (error) {
