...
export default function RequestPage() {
  ...
  const fetchRequests = async () => {
    try {
      const response = await axios.get('http://localhost:8085/api/requests');
  ...
  const handleAddRequest = async (request: Omit<BloodRequest, 'id' | 'createdAt' | 'updatedAt'>) => {
    try {
      await axios.post('http://localhost:8085/api/requests', request);
  ...
  const handleUpdateStatus = async (id: number, status: Status) => {
    try {
      await axios.put(`http://localhost:8085/api/requests/${id}/status?status=${status}`);
      fetchRequests();
    } catch (error) {
