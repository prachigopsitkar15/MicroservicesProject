...
export default function DonorsPage() {
  ...
  const fetchDonors = async () => {
    try {
      const response = await axios.get('http://localhost:8085/api/donors');
  ...
  const handleAddDonor = async (newDonor: Omit<Donor, 'id'>) => {
    try {
      await axios.post('http://localhost:8085/api/donors', newDonor);
  ...
  const handleUpdateDonor = async (updatedDonor: Donor) => {
    try {
      await axios.put(`http://localhost:8085/api/donors/${updatedDonor.id}`, updatedDonor);
  ...
  const handleDeleteDonor = async (id: number) => {
    if (window.confirm('Are you sure you want to delete this donor?')) {
      try {
        await axios.delete(`http://localhost:8085/api/donors/${id}`);
        fetchDonors();
      } catch (error) {
  ...
  const handleDonation = async (id: number) => {
    try {
      await axios.post(`http://localhost:8085/api/donors/${id}/donate`);
      fetchDonors();
    } catch (error) {
